...
import { styled, Button, Accordion, AccordionSummary, AccordionDetails, Alert, Box, Chip, FormControlLabel, Switch, Card, CardContent, CircularProgress, Dialog, DialogContent, Divider, Grid, Paper, Stack, Tab, TextField, Toolbar, Typography, useTheme } from '@mui/material';
import { DataGrid } from '@mui/x-data-grid';
import { TabContext, TabList, TabPanel } from '@mui/lab';
import { ExpandMore as ExpandMoreIcon, Download, Warning as WarningIcon, Error as ErrorIcon, Pending as PendingIcon, Autorenew as AutorenewIcon, TimerOff as TimerOffIcon, SimCardAlert as SimCardAlertIcon } from '@mui/icons-material';
...
// For Active jobs popup box
  ...
  const convertToCSV = (content) => {
    ...
    // Get the headers dynamically from the first row of queuedRows
    const headers = Object.keys(content[0]);

    // Generate CSV data with headers as the first row
    const csvData = content.map(row => headers.map(header => row[header]));
  ...
  const MinimalisticButton = styled(Button)({
                  ...
                  <Box mb={3}>
                    ...
                    <Box mb={2}>
                      {[
                        ...
                        { label: 'Work Dir', value: selectedRow.workdir },
                ...
                {active_loading ? (
                  ...
                  <Box className="data-grid-container" sx={{ height: 520, width: '100%' }}>
                    ...
                    <CSVLink
                        ...
                        filename={"active-jobs-export.csv"}
                        className="hidden"
                        target="_blank"
                        ref={csvLinkRef}
                      />
                ...
                {pending_loading ? (
                 <center><CircularProgress /></center>
                  ...
                  <Box className="data-grid-container" sx={{ height: 520, width: '100%' }}>
                    ...
                    />
                    <CSVLink
                        data={convertToCSV(queuedRows)}
                        filename={"pending-jobs-export.csv"}
                        className="hidden"
                        target="_blank"
                        ref={csvLinkRef}
                      />
                  ...
                  <Box className="data-grid-container" sx={{ height: 520, width: '100%' }}>
                    ...
                    />
                      <CSVLink
                        ...
                        filename={"completed-jobs-export.csv"}
                        className="hidden"
                        target="_blank"
                        ref={csvLinkRef}
                      />
