...
// For Active jobs popup box
  const handleRowClick = async (params) => {
    ...
    const stats = await fetchJobStats('armis2', params.row.id);
    setJobStats(stats);

    const tres = await fetchJobTres('armis2', params.row.id);
    setJobTRES(tres);

    const seff = await fetchSeff('armis2', params.row.id);
    setJobSEFF(seff);
  };
        ...
        <Grid container spacing={2} justifyContent="center">
          <Grid item>
            <Card variant="outlined" sx={{ boxShadow: 3, padding: '1em' }}>
              ...
              <BudgetDisplay cluster={'armis2'} account={searchValue} />
            </Card>
          </Grid>
          <Grid item>
            <Card variant="outlined" sx={{ boxShadow: 3, padding: '1em' }}>
          ...
          <Grid item>
            <Card variant="outlined" sx={{ boxShadow: 3, padding: '1em' }}>
              <PartitionStats clusterName={'armis2'} />
            </Card>
          </Grid>
        </Grid>
